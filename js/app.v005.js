...
function nav(path) {
  ...
  html += `</div></li><li class="nav-item">
      <a class="nav-link" href="${UI.contact_link}" target="_blank">${
    UI.nav_link_4
  }</a>
    </li>${
      UI.show_logout_button
...
function requestListPath(path, params, resultCallback, authErrorCallback) {
  ...
  $("#update").html(
    `<div class='alert alert-info' role='alert'> Connecting...</div></div></div>`
  );
  $.post(path, p, function (data, status) {
    ...
    if (res && res.error && res.error.code == "401") {
      ...
      $("#spinner").remove();
      $("#list").html(
        `<div class='alert alert-danger' role='alert'> Server didn't sent any data. </div></div></div>`
      );
      $("#update").remove();
    ...
    sleep(2000);
    $("#update").html(
      `<div class='alert alert-info' role='alert'> Retrying...</div></div></div>`
    );
    $.post(path, p, function (data, status) {
      var res = jQuery.parseJSON(gdidecode(read(data)));
      if (res && res.error && res.error.code == "401") {
      ...
      $.post(path, p, function (data, status) {
        ...
        $("#update").html(
          `<div class='alert alert-danger' role='alert'> Unable to get data from the server, Something went wrong.</div></div></div>`
        );
        $("#list").html(
          `<div class='alert alert-danger' role='alert'> We were unable to get data from the server.</div></div></div>`
        );
        $("#spinner").remove();
      });
    });
...
function requestSearch(params, resultCallback) {
  ...
  $.post(`/${window.current_drive_order}:search`, p, function (data, status) {
    ...
    $.post(`/${window.current_drive_order}:search`, p, function (data, status) {
      ...
      $.post(
        `/${window.current_drive_order}:search`,
        p,
        ...
        $("#update").html(
          `<div class='alert alert-danger' role='alert'> Unable to get data from the server, Something went wrong. 3 Failures</div></div></div>`
        );
        $("#list").html(
          `<div class='alert alert-danger' role='alert'> We were unable to get data from the server.</div></div></div>`
        );
...
function file(path) {
  ...
  var ext = name
    ...
    .toLowerCase();
  $("#content").html(
    `<div class="d-flex justify-content-center" style="height: 150px"><div class="spinner-border ${UI.loading_spinner_class} m-5" role="status" id="spinner"><span class="sr-only"></span></div></div>`
  );
    ...
    try {
      ...
      if (mimeType == "application/vnd.google-apps.folder") {
        var content =
          ...
          `/" type="button" class="btn btn-primary">Open as Folder</a>
                    </div>
                    </div><br>
                    </div>
                    </div>`;
...
function file_video(path) {
  ...
  var url = UI.second_domain_for_dl
    ...
    : window.location.origin + path;
  var urlvlc = url
    .replace(new RegExp("\\[", "g"), "%5B")
    .replace(new RegExp("\\]", "g"), "%5D")
  ...
  <div class="input-group mb-4">
    ...
    <div class="input-group-append">
      <button onclick="copyFunction()" onmouseout="outFunc()" class="btn btn-outline-info"> <span class="tooltiptext" id="myTooltip">Copy</span> </button>
    </div>
  </div>
  ${
    UI.display_drive_link
  ...
  <a href="${url}" class="btn btn-outline-danger" role="button" aria-pressed="true">Download</a>
  <div class="btn-group text-center">
